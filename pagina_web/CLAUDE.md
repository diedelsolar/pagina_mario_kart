...
- Formato: doble eliminación por país, rama Principal + rama Repechaje, copas
  de 4 carreras (top 2 avanza, SOLO el 3º baja a repechaje, el 4º queda
  eliminado directo).
- Fase 0 ya jugada: Chile 25 inscritos → 20 clasificados (5 peores tiempos
  eliminados, 5 copas de 4 en R1). Perú 12 inscritos → 8 clasificados (3
  peores tiempos + 1 sin tiempo eliminados, 2 copas de 4 en R1, sin pase
  directo/bye).
- "Hurtado De Mendoza Diaz, Valeria Fernanda" (Perú) aparece dos veces con
  gamertags distintos (`valita` y `YuGo`); `valita` quedó eliminada por
  tiempo y `YuGo` sigue clasificada, así que ya no hay ambigüedad.
- Solo se juega los jueves, máximo 2 copas por jueves — repartir R2,
  Repechaje, Finales, etc. en tantos jueves como haga falta.
